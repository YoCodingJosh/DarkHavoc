...
{
    public class LightningText
    {
        static JoshoRandom rand = new JoshoRandom();
        ...
        public void Update()
        {
            ...
            foreach (var particle in textParticles)
            {
                ...
                if (rand.NextInt(boltChance) == 0)
                {
                    ...
                    for (int i = 0; i < 50; i++)
                    {
                        var other = textParticles[rand.NextInt(textParticles.Count)];
        ...
        List<Vector2> CreateParticleText(GraphicsDevice device, SpriteFont font, string text)
        {
            ...
            for (int y = 0; y < height; y += interval)
            {
                ...
                {
                    int tx = Clamp(x + rand.NextInt(-interval / 2, interval / 2), 0, width - 1);
                    int ty = Clamp(y + rand.NextInt(-interval / 2, interval / 2), 0, height - 1);
